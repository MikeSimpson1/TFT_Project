...
def importChamps():
    ...
    for champ in jsonChamps:
        champsList.append(Champion(champ["champion"], champ["cost"], champ["traits"], champ["image"]))
    return champsList
...
def groupByTraits(champ_list, traits_list, printBool):
    groupedTraits = {}
    for x in traits_list:
        groupedTraits[x.name] = []
...
###########################
def main(championsList, traitsList, itemsList):
    testDeck = [championsList[14], championsList[29], championsList[34]]
    print(isTraitGold(testDeck, traitsList[18]))
    groupedChamps = groupByTraits(championsList, traitsList, 0)
...
main(championsList, traitsList, itemsList)
